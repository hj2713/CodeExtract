...
import { savePrompt, saveGithubUrl, getPrompt, getGithubEntries, type GithubEntry } from "./actions";
...
export default function CrudPromptGithubPage() {
	...
	useEffect(() => {
		async function loadInitial() {
			const [loadedPrompt, savedEntries] = await Promise.all([getPrompt(), getGithubEntries()]);
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const [promptResult, githubResult] = await Promise.all([
			savePrompt(prompt),
			githubUrl ? saveGithubUrl(githubUrl) : Promise.resolve({ success: true }),
		]);

		if (!promptResult.success) {
			setMessage({ type: "error", text: `Failed to save prompt: ${promptResult.error}` });
		} else if (!githubResult.success) {
			setMessage({ type: "error", text: `Failed to save GitHub URL: ${githubResult.error}` });
		} else {
			setMessage({ type: "success", text: "Saved successfully!" });
			setSavedPrompt(prompt);
			if ("entry" in githubResult && githubResult.entry) {
				setEntries((prev) => [...prev, githubResult.entry as GithubEntry]);
			}
			setGithubUrl("");
		}
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<div className="space-y-6">
					<section className="rounded-none border p-4">
						<h2 className="mb-4 font-mono font-medium">Add Repository</h2>
				...
				{/* Right column: Saved Repositories */}
				<div className="space-y-4">
					<h2 className="font-mono font-medium">Saved Repositories</h2>
					{entries.length === 0 ? (
						<div className="rounded-none border border-dashed p-8 text-center">
							<p className="font-mono text-xs text-muted-foreground">
								No repositories saved yet. Add a GitHub URL above.
							</p>
						</div>
					) : (
						entries.map((entry) => <GithubRepoViewer key={entry.id} entry={entry} />)
					)}
				</div>
			</div>
		</div>
